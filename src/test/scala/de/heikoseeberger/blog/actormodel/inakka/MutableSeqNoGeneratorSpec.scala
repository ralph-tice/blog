...
final class MutableSeqNoGeneratorSpec extends WordSpec with Matchers with BeforeAndAfterAll {
  import MutableSeqNoGenerator._

  private implicit val system = ActorSystem()

  "A MutableSeqNoGenerator" should {
    "respond to GetNext with the correct SeqNo" in {
      val sender             = TestProbe()
      implicit val senderRef = sender.ref
      val seqNoGenerator     = system.actorOf(Props(new MutableSeqNoGenerator(42)))
